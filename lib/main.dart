...
(function() {
  ...
  label.id = 'uniq-native-google-label';
  label.textContent = 'Google ile giris';
  ...
  var registerCandidates = all.filter(function(el) {
    var t = (el.innerText || '').trim().toLowerCase();
    return isVisible(el) && (t.indexOf('hesabiniz yok') >= 0 || t.indexOf('kayıt ol') >= 0 || t.indexOf('kayit ol') >= 0);
  });
...
(function() {
  ...
  var label = document.getElementById('uniq-native-google-label');
  ...
  if (label) label.textContent = loading ? 'Bekleyiniz...' : 'Google ile giris';
})();
''');
    } catch (_) {
      // Best effort only.
